...
public class ItemTransportalizer extends Item {
    ...
    @Override
    public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int x, int
            y, int z, int side, float px, float py, float pz) {
        if (world.isRemote) {
            return false;
        }
        if (picked == null) {
            ...
            if (rnd.nextInt(2) == 1) {
                world.getBlock(x, y, z).dropBlockAsItem(world, x, y, z, world.getBlockMetadata(x,
                        y, z), 0);
            }
            inv.setInteger("x", x);
            inv.setInteger("y", y);
