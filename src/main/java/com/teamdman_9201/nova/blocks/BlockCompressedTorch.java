...
public class BlockCompressedTorch extends BlockTorch implements ITileEntityProvider {
     ...
     */
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int
            fortune) {
     ...
     */
    public void onBlockPlacedBy(World world, int xPos, int yPos, int zPos, EntityLivingBase
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int p_149749_6_) {
        ...
        if (tile != null) {
            ...
            while (stack.stackSize > 0) {
                ...
                stack.stackSize -= j1;
                EntityItem entityitem = new EntityItem(world, (double) ((float) x + f), (double)
                        ((float) y + f1), (double) ((float) z + f2), new ItemStack(stack.getItem
                        (), j1, stack.getItemDamage()));

                if (stack.hasTagCompound()) {
                    entityitem.getEntityItem().setTagCompound((NBTTagCompound) stack
                            .getTagCompound().copy());
                }
