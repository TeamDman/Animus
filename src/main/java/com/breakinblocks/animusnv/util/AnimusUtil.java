...
public class AnimusUtil {
    ...
    public static AraVitaeTile getNearbyAltar(Level level, AreaDescriptor area, BlockPos offset, BlockPos previous) {
        BlockEntity tile;
        if (area.isWithinArea(previous) && (tile = level.getBlockEntity(previous)) instanceof AraVitaeTile) {
            return (AraVitaeTile) tile;
        }

        Optional<BlockEntity> found = area.getContainedPositions(offset).stream()
            .map(level::getBlockEntity)
            .filter(e -> e instanceof AraVitaeTile)
            .findFirst();

        area.resetCache();
        return (AraVitaeTile) found.orElse(null);
    }
}
