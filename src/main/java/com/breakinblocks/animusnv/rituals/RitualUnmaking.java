...
public class RitualUnmaking extends Ritual {
    ...
    public void performRitual(IMasterRitualStone mrs) {
        ...
        List<ItemEntity> books = itemList.stream()
            .filter(e -> !e.isRemoved() && e.getItem().is(Items.BOOK)).toList();
        ...
        for (ItemEntity itemEntity : itemList) {
            ...
            if (stack.is(Items.ENCHANTED_BOOK)) {
                ...
                if (!consumeBooks(books, enchants.size())) continue;
                ...
                if (!consumeBooks(books, enchantments.size())) continue;
    ...
    private static boolean consumeBooks(List<ItemEntity> books, int count) {
        if (books.stream().mapToInt(e -> e.getItem().getCount()).sum() < count) return false;
