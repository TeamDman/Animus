...
/** Eligibility is read-only; payment occurs immediately before the real mana deduction. */
public final class SpellCastingHandler {
    ...
    public static boolean payAndSupplyMana(Player player, int manaCost) {
        ...
        if (data.getMana() >= manaCost) return true;
        Payment payment = payment(player, manaCost);
        if (payment == null || player.level().isClientSide()) return false;
        int drained = payment.network.syphon(AnimaTicket.create(payment.ev));
        if (drained != payment.ev) {
            if (drained > 0) payment.network.add(AnimaTicket.create(drained), Integer.MAX_VALUE);
            return false;
        }
    ...
    private static Payment payment(Player player, int manaCost) {
        if (!AnimusConfig.ironsSpells.enableEVCasting.get()) return null;
        ...
        if (orb.isEmpty() && AnimusConfig.ironsSpells.requireBloodOrb.get()) return null;
        ...
        if (cost > Integer.MAX_VALUE) return null;
