...
public class ItemSanguineScroll extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        try {
            if (reserved != evCost) return InteractionResultHolder.fail(stack);
            accepted = spell.attemptInitiateCast(stack, spellLevel, level, player, CastSource.SCROLL, true, "sanguine_scroll");
            if (!accepted) return InteractionResultHolder.fail(stack);
            ...
            if (!accepted && reserved > 0) network.add(AnimaTicket.create(reserved), Integer.MAX_VALUE);
        }
    }
