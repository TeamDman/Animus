...
public final class CullingHelper {
     ...
     * killBoss config enabled, 100+ destructive demon will, and sufficient LP.
     * Returns true if the boss was made vulnerable and can be killed.
     *
     * @param entity         the boss entity
     * @param currentAmount  current destructive demon will amount
     * @param currentEssence current LP in the soul network
     * @param requiredEssence the LP needed (boss cost + refresh cost * entity count)
     * @return true if the boss was made vulnerable
     */
    public static boolean tryMakeBossVulnerable(LivingEntity entity, double currentAmount, int currentEssence, int requiredEssence) {
        if (AnimusConfig.rituals.killBoss.get() && currentAmount > 99 && currentEssence >= requiredEssence) {
            if (AnimusConfig.rituals.cullingDebug.get()) {
