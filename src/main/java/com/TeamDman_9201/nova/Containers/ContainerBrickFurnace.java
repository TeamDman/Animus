...
public class ContainerBrickFurnace extends Container {
    ...
    public ContainerBrickFurnace(InventoryPlayer par1InventoryPlayer, TileBrickFurnace
        ...
        this.addSlotToContainer(new Slot(tileEntityBrickFurnace, 1, 56, 53));
        this.addSlotToContainer(new SlotFurnace(par1InventoryPlayer.player,
                tileEntityBrickFurnace, 2, 116, 35));
        int i;
        // Inventory Creation
        for (i = 0; i < 3; ++i) {
            for (int j = 0; j < 9; ++j) {
                this.addSlotToContainer(new Slot(par1InventoryPlayer, j + i * 9 + 9, 8 + j * 18,
                        84 + i * 18));
            }
        }
    ...
    public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int par2) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (par2 == 2) {
                ...
                } else if (par2 >= 30 && par2 < 39 && !this.mergeItemStack(itemstack1, 3, 30,
                        false)) {
                    return null;
                }
