...
//							world.setBlock(x, y + 1, z, Blocks.torch);
							world.setBlock(x, y+1, z, Block.getBlockFromItem(tileLightManipulator.getStackInSlot(i).getItem()));
	...
	public ArrayList<int[]> getBlocksInChunk(World world, boolean useSkyLightLevel, int radius) {
		...
		System.out.printf("Geting blocks at (%d,%d,%d) chunks (%d,%d) radius (%d)", startX, startY, startZ, startChunkX, startChunkZ, radius);
		...
		System.out.printf("generation took %dms to collect %dblocks.\n", System.currentTimeMillis() - tStart, chunkContents.size());
