 ...
 */
public class NOVAWorldGenTree extends WorldGenAbstractTree {
  ...
  public NOVAWorldGenTree(boolean arg) {
    super(arg);
  }
...
//    }
      ...
      if (f < 0.0F) {
        ...
        for (double d0 = 0.5D; j1 < i; ++j1) {
          ...
          if (this.checkBlockLine(aint1, aint2) == -1) {
            ...
            double
                d3 =
                Math.sqrt(Math.pow((double) Math.abs(this.basePos[0] - aint1[0]), 2.0D) + Math
                    .pow((double) Math.abs(this.basePos[2] - aint1[2]), 2.0D));
  ...
  void placeBlockWithCoords(int posX, int posY, int posZ, float p_150529_4_, byte p_150529_5_,
                            Block leaf) {
    ...
    for (count[p_150529_5_] = coords[p_150529_5_]; i1 <= l; ++i1) {
      ...
      while (j1 <= l) {
        double
            d0 =
            Math.pow((double) Math.abs(i1) + 0.5D, 2.0D) + Math
                .pow((double) Math.abs(j1) + 0.5D, 2.0D);
        ...
        if (d0 > (double) (p_150529_4_ * p_150529_4_)) {
          ...
          if (!block1.isAir(worldObj, count[0], count[1], count[2]) && !block1
              .isLeaves(worldObj, count[0], count[1], count[2])) {
            ++j1;
          } else {
  ...
  float layerSize(int p_76490_1_) {
    if ((double) p_76490_1_ < (double) ((float) this.heightLimit) * 0.3D) {
      ...
      if (f1 == 0.0F) {
        ...
        f2 =
            (float) Math
                .sqrt(Math.pow((double) Math.abs(f), 2.0D) - Math.pow((double) Math.abs(f1), 2.0D));
      }
  ...
  float leafSize(int size) {
    return size >= 0 && size < this.leafDistanceLimit ? (
        size != 0 && size != this.leafDistanceLimit - 1 ? 3.0F : 2.0F) : -1.0F;
  }
  ...
  void placeBlock(int[] p_150530_1_, int[] p_150530_2_, Block p_150530_3_) {
    ...
    if (aint2[b1] != 0) {
      ...
      for (int j = aint2[b1] + b4; i != j; i += b4) {
        ...
        this.setBlockAndNotifyAdequately(this.worldObj, aint3[0], aint3[1], aint3[2], p_150530_3_,
                                         b5);
      }
    }
  }
  ...
  boolean validTreeLocation() {
    ...
    boolean
        isSoil =
        block.canSustainPlant(worldObj, basePos[0], basePos[1] - 1, basePos[2], ForgeDirection.UP,
                              (BlockSapling) NOVA.blockSapling);
    if (!isSoil) {
      return false;
    } else {
